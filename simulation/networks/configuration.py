...
class GroupConfiguration:
    ...
    def from_dict(cls, d: dict) -> "GroupConfiguration":
        ...
        return cls(
            ...
            if "connection_quality" in d
            else None,
            d["ipv6"] if "ipv6" in d else False,
            d["additional_features"] if "additional_features" in d else [],
            d["clear_default_features"]
            if "clear_default_features" in d
            else False,
    ...
    def get_daemon_args_str(self) -> str:
        ...
        args_str = " ".join(
            [f"--{arg.replace('_', '-')} {args[arg]}" for arg in args]
        )
...
class Configuration:
    ...
    def from_yaml(cls, yaml_path: str) -> "Configuration":
        ...
        assert (
            "groups" in parameters and type(parameters["groups"]) == list
        ...
        return cls(
            ...
            if "num_search_tests" in parameters
            else None,
            parameters["debug"] if "debug" in parameters else True,
    ...
    def run(self, output_directory: str) -> dict:
        ...
        log.info(
            f"Search results: {percentage_success * 100}% success, "
        ...
        drawer.draw_main_graph(network_logs, main_graph_path)
        ...
        for i in range(len(search_results)):
            from_key_id, to_key_id, result, message_id, num_requests, search_time_sec = search_results[
                i
            ]
            ...
            results_dict["search_results"].append(
                dict(
