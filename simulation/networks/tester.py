...
class SearchResult:
    ...
    def average_search_time_sec(self) -> float:
        ...
        if not successful_search_times_sec:
            return 0.0
        return sum(successful_search_times_sec) / len(
            successful_search_times_sec
        )


def test_search(
    network: Network, from_key_id: str, to_key_id: str
    ...
    try:
        ...
        output = network.exec_command(
            from_key_id, ["/root/kipa_cli", "search", "--key-id", to_key_id]
        )
...
def test_all_searches(network: Network) -> SearchResult:
    ...
    for k1, k2 in itertools.permutations(keys, 2):
        success, message_id, num_requests, search_time_sec = test_search(
            network, k1, k2
        )
        results.add_result(
...
def sample_test_searches(
    network: Network, num_searches: int = None
) -> SearchResult:
