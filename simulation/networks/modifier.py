...
def connect_node(
    network: Network, connector_key_id: str, connectee_key_id: str
) -> None:
    ...
    output = network.exec_command(
        ...
        [
            ...
            connectee_key_id,
            "--address",
            network.get_address(connectee_key_id),
        ],
    )
    if "Connect successful" not in output:
...
def fake_poor_connection(
    network: Network, loss: float, delay: float, rate: float
) -> None:
    log.debug(
